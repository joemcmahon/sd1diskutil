...
mod tests {
    ...
    // Regression: FAT must start at block 5 (hardware format). An earlier version
    // used block 4 (based on emulator-created disk with wrong layout), causing the
    // SD-1 to see all files as missing.
    #[test]
    fn fat_starts_at_block_5_hardware_compatible() {
        let mut img = blank();
        // Write a chain at block 23 and verify the bytes land in block 5
        FileAllocationTable::set_chain(&mut img, &[23u16, 24]);
        // Block 5 offset 69 = entry 23, byte 0 (should be 0x00 then 0x00 then 0x18=24)
        let off = 5 * 512 + 23 * 3;
        assert_eq!(
            &img.data[off..off + 3], &[0x00, 0x00, 24],
            "FAT chain for block 23 must be stored in disk block 5 (hardware format)"
        );
        // Block 4 at the same relative offset must be untouched (all zero)
        let off4 = 4 * 512 + 23 * 3;
        assert_eq!(
            &img.data[off4..off4 + 3], &[0x00, 0x00, 0x00],
            "block 4 must NOT contain FAT data (only block 5 does on real hardware)"
        );
    }
